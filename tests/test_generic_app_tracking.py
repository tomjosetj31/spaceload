...
from spaceload.adapters.vpn.tunnelblick import TunnelblickAdapter
from spaceload.adapters.wm.base import WMWindow
...
class TestWindowSnapshotPoller:
    def _make_poller(self):
        ...
        from spaceload.daemon.server import WindowSnapshotPoller
        actions = []
    ...
    def test_no_wm_fallback_records_new_app(self):
        ...
        with patch("spaceload.daemon.server._get_running_foreground_apps",
...
class TestReplayerAppOpen:
    ...
    def test_app_open_places_in_workspace(self, capsys):
        ...
        with patch("subprocess.run", return_value=_proc(returncode=0)), \
             patch("spaceload.replayer.replayer.time") as mock_time, \
             patch("spaceload.adapters.wm.registry.WorkspaceManagerRegistry") as mock_reg:
