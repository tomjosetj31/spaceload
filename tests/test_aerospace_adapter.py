...
from spaceload.daemon.server import BrowserPoller, IDEPoller, TerminalPoller
...
class TestBrowserPollerWorkspaceEnrichment:
    def test_workspace_added_to_browser_tab_open_event(self):
        ...
        with patch("spaceload.daemon.server.BrowserAdapterRegistry", return_value=mock_registry), \
...
class TestReplayerWorkspacePlacement:
    def test_browser_tab_open_moves_to_workspace(self, capsys):
        ...
        with patch("spaceload.replayer.replayer.time.sleep"):
            replayer.replay()
