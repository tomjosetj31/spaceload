...
Usage (internal — spawned by the CLI):
    python -m spaceload.daemon.server <workspace_name> [--db <db_path>]
"""
...
_LOG_PATH = _SPACELOAD_DIR / "daemon.log"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--db",
        default=str(_SPACELOAD_DIR / "spaceload.db"),
        help="Path to the SQLite database (default: ~/.spaceload/spaceload.db)",
    )
    parser.add_argument(
        "--include-open",
        action="store_true",
...
if __name__ == "__main__":
    _setup_logging()
    logger.info("=" * 60)
    logger.info("spaceload daemon starting")
    logger.info("=" * 60)
    args = _parse_args()
