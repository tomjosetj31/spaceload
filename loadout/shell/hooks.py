...
These hooks integrate with zsh/bash to report commands to the loadout daemon.
"""
...
ZSH_HOOK = r'''
# loadout shell integration for zsh
# Add to your .zshrc: eval "$(loadout shell-hook zsh)"

_loadout_socket="${HOME}/.loadout/daemon.sock"

# Track the command before execution
_loadout_preexec() {
    # Skip if daemon socket doesn't exist (not recording)
    [[ -S "$_loadout_socket" ]] || return
    ...
    # Redirect all output to /dev/null to avoid printing {"status": "ok"}
    {
        printf '{"command":"record_action","action":{"type":"terminal_command","timestamp":"%s","app":"shell","session_id":"%s","directory":"%s","cmd":"%s"}}\n' \
            "$timestamp" "$session_id" "$current_dir" "${1//\"/\\\"}" | nc -U "$_loadout_socket" >/dev/null 2>&1
    } &!
}

# Hook into zsh
autoload -Uz add-zsh-hook
add-zsh-hook preexec _loadout_preexec
'''

# Bash hook script - uses DEBUG trap and PROMPT_COMMAND
BASH_HOOK = r'''
# loadout shell integration for bash
# Add to your .bashrc: eval "$(loadout shell-hook bash)"

_loadout_socket="${HOME}/.loadout/daemon.sock"
_loadout_last_command=""
...
_loadout_debug_trap() {
    ...
    [[ -z "$cmd" ]] && return
    [[ "$cmd" == "$_loadout_last_command" ]] && return
    
    # Skip internal commands
    [[ "$cmd" == _loadout_* ]] && return
    
    _loadout_last_command="$cmd"
    ...
    {
        printf '{"command":"record_action","action":{"type":"terminal_command","timestamp":"%s","app":"shell","session_id":"%s","directory":"%s","cmd":"%s"}}\n' \
            "$timestamp" "$session_id" "$current_dir" "${cmd//\"/\\\"}" | nc -U "$_loadout_socket" >/dev/null 2>&1
    } &
}

# Set up the DEBUG trap
trap '_loadout_debug_trap' DEBUG
'''
